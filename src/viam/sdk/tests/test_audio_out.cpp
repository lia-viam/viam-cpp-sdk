...
BOOST_AUTO_TEST_CASE(test_play_stream) {
    ...
    client_to_mock_pipeline<AudioOut>(mock, [mock](AudioOut& client) {
        ...
        std::size_t idx = 0;
        client.play_stream(info, [&]() -> std::optional<std::vector<std::uint8_t>> {
            if (idx >= chunks.size()) {
                return std::nullopt;
            }
            return chunks[idx++];
...
BOOST_AUTO_TEST_CASE(test_play_stream_no_chunks) {
    ...
    client_to_mock_pipeline<AudioOut>(mock, [mock](AudioOut& client) {
        ...
        // chunk_source returns std::nullopt immediately — empty stream.
        client.play_stream(
            info, []() -> std::optional<std::vector<std::uint8_t>> { return std::nullopt; });
